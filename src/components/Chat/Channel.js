...
const Channel = async ({ address }) => {
  const db = getFirestore(firebase);
  const messagesRef = collection(db, "messages");
  const messages = await getDoc(messagesRef);
  //   const messages = query(messagesRef, orderBy("createdAt", "desc"), limit(100));

  console.log("hellooo", messages?.data());
